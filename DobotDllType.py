...
def load():
    return cdll.LoadLibrary("E:\MCILAB\Dobot\dobot-production-line-handler-2-final-edition\DobotDll.dll")
...
def SetHOMECmd(api, temp=0, isQueued=0):
    cmd = HOMECmd()
    cmd.temp = temp
    queuedCmdIndex = c_uint64(0)
    while (True):
...
def SetColorSensor(api, isEnable, colorPort):
    ...
    while (True):
        result = api.SetColorSensor(enable, port)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetHOMECmdEx(api, temp=0, isQueued=0):
...
def SetEMotorEx(api, index, isEnabled, speed, isQueued=0):
    ...
    print ret[0], GetQueuedCmdCurrentIndex(api)[0]
    while (True):
        if ret[0] <= GetQueuedCmdCurrentIndex(api)[0]:
            break
        dSleep(5)
...
def SetInfraredSensor(api, isEnable, infraredPort):
    ...
    while (True):
        result = api.SetInfraredSensor(enable, port)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
